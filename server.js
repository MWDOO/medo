...
setInterval(() => {
  http.get(`http://pot-syste.glitch.me/`);
}, 280000);

const Discord = require("discord.js"); //ABN | M e D o O
const client = new Discord.Client(); //ABN | M e D o O
const moment = require("moment");
const zalgo = require("zalgolize"); //ABN | M e D o O
const math = require("math-expression-evaluator"); //ABN | M e D o O
const figlet = require("figlet"); //ABN | M e D o O
const fs = require("fs"); //ABN | M e D o O
const ms = require("ms"); //ABN | M e D o O
const prefix = "+";
...
client.on("message", msg => {
  ...
  if (msg.content === "هاي") {
    //ABN | M e D o O
    msg.reply("** :wink: وِلِـكُمِـ ﻧَوِرُتْ   :sparkling_heart:**"); //ABN | M e D o O
  }
});
...
client.on("message", msg => {
  ...
  if (msg.content === "ميدو") {
    //ABN | M e D o O
    msg.reply("**لمعلم ميدو قصدك**"); //ABN | M e D o O
  }
});
...
client.on("message", msg => {
  ...
  if (msg.content === "اوس") {
    //ABN | M e D o O
    msg.reply("**القيادة بويه**"); //ABN | M e D o O
  }
});

client.on("message", msg => {
  //ABN | M e D o O
  if (msg.content === "مين صنعك") {
    //ABN | M e D o O
    msg.reply("**المعلم:- ! M e d o狼 非#2597 😉**"); //ABN | M e D o O
  }
});
...
client.on("message", msg => {
  ...
  if (msg.content === "سلام عليكم") {
    //ABN | M e D o O
    msg.reply("**و عليكم السلام**"); //ABN | M e D o O
  }
});
...
client.on("message", msg => {
  ...
  if (msg.content === "احبك") {
    //ABN | M e D o O
    msg.reply("**حبك برص يا سافل😒**"); //ABN | M e D o O
  }
});
...
client.on("message", msg => {
  ...
  if (msg.content === "هلا") {
    //ABN | M e D o O
    msg.reply("**هلا بيك :heart: **"); //ABN | M e D o O
  }
});
...
client.on("message", message => {
  ...
  if (message.content.startsWith(prefix + "روم")) {
    //ABN | M e D o O
    let args = message.content.split(" ").slice(1); //ABN | M e D o O
    var nam = args.join(" ");
    ...
    if (!nam)
      return message.channel
        ...
        .then(msg => msg.delete(10000)); //ABN | M e D o O
    message.guild
      .createChannel(nam, "voice")
      .then(c => setTimeout(() => c.delete(), 120000)); //ABN | M e D o O
    message.channel
...
client.on("message", message => {
  ...
  if (message.content.startsWith(prefix + "emoji")) {
    //ABN | M e D o O
    if (message.author.bot) return; //ABN | M e D o O
    ...
    if (emojiid.length < "18" || emojiid.length > "18" || isNaN(emojiid))
      return message.channel.send(`- Usage  //ABN | M e D o O
${prefix}emoji <EmojiID>`);
    else
      message.channel.send(
        "This is the emoji that you requested:-", //ABN | M e D o O
        {
          files: [`https://cdn.discordapp.com/emojis/${emojiid}.png`] //ABN | M e D o O
        }
      );
  }
});
...
client.on("message", message => {
  ...
  if (message.channel.type === "dm") {
    ...
    message.channel.startTyping(); //ABN | M e D o O
    Timeout(() => {
      //ABN | M e D o O
      message.channel.stopTyping(); //ABN | M e D o O
    }, Math.random() * (1 - 3) + 1 * 1000); //ABN | M e D o O
  }
});

const clans = {}; //ABN | M e D o O
const system = {}; //ABN | M e D o O
const level = {}; //ABN | M e D o O

client.on("message", async message => {
  //ABN | M e D o O
  if (message.author.bot) return; //ABN | M e D o O
  if (message.channel.type === "dm") return; //ABN | M e D o O

  let args = message.content.split(" "); //ABN | M e D o O
  let random = Math.floor(Math.random() * 5) + 2; //ABN | M e D o O
  let author = message.author;

  let xpLeft; //ABN | M e D o O
  let nameClan; //ABN | M e D o O
  let membersClan = []; //ABN | M e D o O
  let levelClan = 0; //ABN | M e D o O
  if (!system[author.id])
    system[author.id] = {
      clan: "None",
  ...
  if (!level[author.id]) level[author.id] = { level: 1, xp: 1 }; //ABN | M e D o O

  level[author.id].xp += +random; //ABN | M e D o O
  if (level[author.id].xp >= 300) {
    //ABN | M e D o O
    if (level[author.id].xp > 300) xpLeft = level[author.id].xp - 300; //ABN | M e D o O
    level[author.id] = {
      //ABN | M e D o O
      level: level[author.id].level + 1, //ABN | M e D o O
      xp: xpLeft //ABN | M e D o O
    };
  }
  if (message.content.startsWith(prefix + "clan")) {
    //ABN | M e D o O
    if (message.content.split(" ")[0] !== `${prefix}clan`) return; //ABN | M e D o O

    if (!args[1] || (args[1] && args[1] === "info")) {
      //ABN | M e D o O
      let embed = new Discord.RichEmbed()
        .setAuthor("الكلانات", message.author.avatarURL) //ABN | M e D o O
        .setDescription(
  ...
  - \`${prefix}clan room\`: لعمل روم شات او كتابي بأسم الكلان ( صاحب الكلان فقط ) ء`
        ...
        .setFooter(message.author.username, message.author.avatarURL); //ABN | M e D o O
    ...
    if (args[1] && args[1] === "create") {
      ...
      let awaited = await message.channel
        ...
        .then(collected => {
          ...
          collected
            ...
            .delete()
            .catch();
          nameClan = collected.first().content; //ABN | M e D o O
        });

      m = await m.edit("**# جارى عمل الكلان**"); //ABN | M e D o O
      awaited = await setTimeout(async () => {
        ...
        let members = membersArray.nameClan.array; //ABN | M e D o O
        members.push(message.author.id);
        clans[nameClan] = {
          name: nameClan,
          createdAt: new Date().toLocaleString(), //ABN | M e D o O
          level: levelClan,
          creator: message.author.id, //ABN | M e D o O
          members: members, //ABN | M e D o O
          applylist: [], //ABN | M e D o O
          admins: [] //ABN | M e D o O  //ABN | M e D o O
        };

        system[author.id] = {
          //ABN | M e D o O
          clan: nameClan, //ABN | M e D o O
          joinedAt: new Date().toLocaleString(), //ABN | M e D o O
          clanLevel: 0, //ABN | M e D o O
          creator: message.author.id //ABN | M e D o O
        };

        m = await m.edit("**# تم عمل الكلان بنجاح**"); //ABN | M e D o O
      }, 2300);
    }
    if (args[1] && args[1] === "invite") {
      //ABN | M e D o O
      if (!system[author.id])
        return message.channel.send("**# أنت لست بكلان**"); //ABN | M e D o O
      ...
      let mention = message.mentions.users.first(); //ABN | M e D o O
      if (!mention) return message.channel.send("**# منشن شخص لدعوته للكلان**"); //ABN | M e D o O
      if (clans[clan].members.includes(mention.id))
        return message.channel.send("**# هذا العضو بالكلان بالفعل**"); //ABN | M e D o O
      if (clans[clan].members.length === 10)
        return message.channel.send(
          "**# هذا الكلان وصل للحد الاقصى من الاعضاء يمكنك**"
        ); //ABN | M e D o O
      //ABN | M e D o O  //ABN | M e D o O  //ABN | M e D o O  //ABN | M e D o O  //ABN | M e D o O
      ...
      let awaiting = await message.channel
        ...
        .then(collected => {
          ...
          if (collected.first().content === "نعم") {
            //ABN | M e D o O
            clans[clan].members.push(mention.id); //ABN | M e D o O

            system[author.id].members += 1; //ABN | M e D o O
            ...
            system[mention.id] = {
              ...
              joinedAt: new Date().toLocaleString(), //ABN | M e D o O
              clanLevel: 0,
              creator: clans[clan].creator //ABN | M e D o O
            };

            message.channel.send(`**${message.author} # تم قبول الدعوة**`); //ABN | M e D o O
          } //ABN | M e D o O
          if (collected.first().content === "لا") {
            //ABN | M e D o O
            message.channel.send(`**${message.author} # تم رفض الدعوة**`); //ABN | M e D o O
          } else if (
            collected.first().content !== "نعم" &&
            collected.first().content !== "لا"
          ) {
            //ABN | M e D o O
            return message.channel.send("**# يجب عليك كتابة `نعم` أو `لا`**"); //ABN | M e D o O
          } //ABN | M e D o O
        }); //ABN | M e D o O
    } //ABN | M e D o O
    if (args[1] && args[1] === "stats") {
      ...
      let clan = system[author.id].clan; //ABN | M e D o O
      let embed = new Discord.RichEmbed() //ABN | M e D o O
        .setAuthor(
          `${message.author.username} || الكلانات`,
          message.author.avatarURL
        ) //ABN | M e D o O
        .setDescription(`الكلان || \`${clan.toString()}\``); //ABN | M e D o O
      embed.addField("» اسم الكلان", clan, true); //ABN | M e D o O
      embed.addField("» تاريخ عمل الكلان", clans[clan].createdAt, true); //ABN | M e D o O
      embed.addField("» تاريخ دخول الكلان", system[author.id].joinedAt, true); //ABN | M e D o O
      embed.addField("» صاحب الكلان", `<+${clans[clan].creator}>`, true); //ABN | M e D o O
      embed.addField("» لفل الكلان", clans[clan].level, true); //ABN | M e D o O
      embed.addField("» عدد اعضاء الكلان", clans[clan].members.length, true); //ABN | M e D o O
      embed.addField(
        "» عدد التقديمات للكلان",
        clans[clan].applylist.length,
        true
      ); //ABN | M e D o O
      embed.addField("» عدد الادمنية بالكلان", clans[clan].admins.length, true); //ABN | M e D o O
      embed.addField(
        "» اعضاء الكلان",
        `${prefix}clan list || يظهرلك رسالة بها اعضاء الكلان`
      ); //ABN | M e D o O
      message.channel.send(embed); //ABN | M e D o O
    }
    if (args[1] && args[1] === "join") {
      //ABN | M e D o O  //ABN | M e D o O
      let clanName = message.content
        .split(" ")
        .slice(2)
        .join(" "); //ABN | M e D o O
      if (system[author.id].clan !== "None")
        return message.channel.send("**# يجب أن لا تكون بكلان**"); //ABN | M e D o O
      if (!args[2])
        return message.channel.send("**# يجب عليك كتابة اسم الكلان**"); //ABN | M e D o O
      if (!clans[clanName])
        return message.channel.send("**# هذا الكلان غير موجود**"); //ABN | M e D o O
      if (clans[clanName].applylist.includes(message.author.id))
        return message.channel.send("**# لقد قدمت على دخول هذا الكلان مسبقا"); //ABN | M e D o O
      //ABN | M e D o O  //ABN | M e D o O  //ABN | M e D o O
      clans[clanName].applylist.push(message.author.id); //ABN | M e D o O
      message.channel.send(
    ...
    if (args[1] && args[1] === "accept") {
      ...
      clans[system[author.id].clan].applylist.shift(mention.id); //ABN | M e D o O
      clans[system[author.id].clan].members.push(mention.id); //ABN | M e D o O
      let clan = system[author.id].clan; //ABN | M e D o O
    ...
    if (args[1] && args[1] === "decline") {
      ...
      system[mention.id] = {
        //ABN | M e D o O
        clan: clans[system[author.id].clan], //ABN | M e D o O
        joinedAt: new Date().toLocaleString(),
        clanLevel: 0 //ABN | M e D o O
      };
    ...
    if (args[1] && args[1] === "promote") {
      ...
      if (system[mention.id].clan === "None")
        return message.channel.send("**# هذا الشخص ليس بكلان**"); //ABN | M e D o O
      if (!clans[system[author.id].clan].members.includes(mention.id))
        return message.channel.send("**# هذا الشخص ليس بالكلان**"); //ABN | M e D o O
      if (clans[system[author.id].clan].admins.includes(mention.id))
        return message.channel.send("**# هذا العضو لديه ادمن بالفعل**"); //ABN | M e D o O
      if (mention.id === message.author.id)
        return message.channel.send("**# لا يمكنك اعطاء نفسك ترقية**"); //ABN | M e D o O

      clans[system[author.id].clan].admins.push(mention.id); //ABN | M e D o O
    ...
    if (args[1] && args[1] === "demote") {
      ...
      if (clans[system[author.id].clan].creator !== message.author.id)
        return message.channel.send("**# هذا الأمر لضاحب الكلان فقط**"); //ABN | M e D o O
      if (!mention)
        return message.channel.send(
          "**# يجب عليك منشنة عضو بالكلان لأعطائه الترقية**"
        ); //ABN | M e D o O
      ...
      if (!clans[system[author.id].clan].members.includes(mention.id))
        return message.channel.send("**# هذا الشخص ليس بالكلان**"); //ABN | M e D o O
      if (!clans[system[author.id].clan].admins.includes(mention.id))
        return message.channel.send("**# هذا الشخص ليس ادمن بالكلان**"); //ABN | M e D o O
      if (mention.id === message.author.id)
        return message.channel.send("**# لا يمكنك اعطاء نفسك ترقية**"); //ABN | M e D o O
      //ABN | M e D o O
      clans[system[author.id].clan].admins.shift(mention.id); //ABN | M e D o O
...
client.on("message", message => {
  ...
  if (message.isMentioned(client.user)) {
    message.reply("**قليل ادب لا تمنشن**");
  }
});
      ...
      .then(function(m) {
        setTimeout(function() {
          m.edit({
            embed: new Discord.RichEmbed()
              .setTitle(
                "[" + virusname + "]: Loading Discord Virus [تهكرت خخخخخخخ] 1%"
              )
              .setColor(0xff0000)
          });
        }, 1000);
...
client.on("guildMemberAdd", member => {
  member
    ...
    .then(function(channel) {
      return channel.send(`**😉 نورت السيرفر يا جميل 😉
💕🌷شرفتنا في سيرفرنا الجميل زيك🌷💕**
:اسم العضو  ${member}:crown:  
  ...
  if (message.content.startsWith("+bot")) {
    message.channel.send({
      embed: new Discord.RichEmbed()
        ...
        .setColor("RANDOM")
        .setTitle("Info 『EGY』 ヅO̷M̷A̷R̷  K̷I̷R̷A̷ヅ.Bot.")
        .addField(
          "``My Ping``",
...
client.on("message", message => {
  if (message.channel.type == "dm") return;
  if (message.content.startsWith("رسالة بوت")) {
    var attentions = {};
...
const devs = ["628613027914055691", "624388202094133271"];
...
client.on("message", message => {
  if (message.content.startsWith("تحدي")) {
    ...
    var embed = new Discord.RichEmbed()
      ...
      .setThumbnail(message.author.avatarURL)
      .addField(
        "『EGY』 ヅO̷M̷A̷R̷  K̷I̷R̷A̷ヅ#0001.Bot",
...
client.on("message", message => {
  ...
  if (message.content.startsWith(prefix + "cat")) {
    ...
    var cat = new Discord.RichEmbed()
      ...
      .setColor(0xd3d0c4)
      .setFooter(`『EGY』 ヅO̷M̷A̷R̷  K̷I̷R̷A̷ヅ#0001.Bot`);
    message.channel.sendEmbed(cat);
  }
});
  ...
  if (message.content.startsWith(prefix + "help")) {
    ...
    message.channel.send(RpsEmbed).then(msg => {
      ...
      reaction1.on("collect", r => {
        const embed = new Discord.RichEmbed()
          .setThumbnail(
            "https://cdn.discordapp.com/attachments/654814836521435143/665982713404194828/group-03_copy-512.png"
          )
          .setColor("#181726").setDescription(`
...
『+infRole Role name ====> يوريك معلومات رتبة باسمها او الايدي حقها
...
`);
      ...
      reaction2.on("collect", r => {
        const embed = new Discord.RichEmbed()
          .setThumbnail(
            "https://cdn.discordapp.com/attachments/654814836521435143/665992943831875594/unnamed.png"
          )
          .setColor("#fc0303").setDescription(`
...
`);
      ...
      reaction3.on("collect", r => {
        const embed = new Discord.RichEmbed()
          .setThumbnail(
            "https://cdn.discordapp.com/attachments/654814836521435143/665984106361913346/images_14.png"
          )
          .setColor("#ffd700").setDescription(`
...
『+لعبة مريم ====> مريم
『+عواصم
『+يعطيك عقابات قاسية ====> صراحة
`);
        message.author.sendEmbed(embed);
        message.reply("تم ارسالك بلخاص");
      });
      reaction3.on("collect", r => {
        const embed = new Discord.RichEmbed().setColor("#4d99e6")
          .setDescription(`
...
client.on("message", message => {
  if (!message.channel.guild) return;
  if (message.content.startsWith("ارسال")) {
    if (!message.channel.guild)
      return message.channel
...
client.login("NjYzNzQzNjg5MzM1NTA0OTA2.XhM9ow.FgTNwGS1cTRmOi7CjlM0IsfWGuY");
